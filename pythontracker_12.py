...
def index():
    ...
    if data_frames:
        ...
        # Check for duplicates and print their Hostnames
        duplicates = combined_df[combined_df.duplicated(subset='Peer ID', keep=False)]
        if not duplicates.empty:
            print("Duplicate Peer IDs found:")
            for peer_id, group in duplicates.groupby('Peer ID'):
                hostnames = group['Hostname'].unique()
                print(f"Peer ID: {peer_id}, Hostnames: {', '.join(hostnames)}")
        else:
            print("No duplicate Peer IDs found.")

        # Remove duplicates and keep only the most recent balance for each Peer ID
        latest_balances = combined_df.groupby('Peer ID').agg({
            'Balance': 'last',
            'Hostname': 'last'  # Ensure Hostname is included correctly
        }).reset_index()
        ...
        last_1440_quil_per_day = last_1440_quil_per_day.reindex(latest_balances['Peer ID']).fillna(0)
        last_1440_quil_per_hour = last_1440_quil_per_hour.reindex(latest_balances['Peer ID']).fillna(0)

        # Add 24-Hour Quil Per Hour and 24-Hour Quil Per Day to latest_balances
        latest_balances['24-Hour Quil Per Day'] = last_1440_quil_per_day.values
        latest_balances['24-Hour Quil Per Hour'] = last_1440_quil_per_hour.values
        ...
        quil_per_minute = quil_per_minute.reindex(latest_balances['Peer ID']).fillna(0)
        quil_per_hour = quil_per_hour.reindex(latest_balances['Peer ID']).fillna(0)
        quil_per_day = quil_per_day.reindex(latest_balances['Peer ID']).fillna(0)
