...
def index():
    ...
    if data_frames:
        ...
        # Also, extract and keep the 'Hostname' column
        latest_balances['Hostname'] = combined_df.groupby('Peer ID')['Hostname'].last()

        # Rest of the metrics calculations and sorting...
        # (same as original code)

        # Prepare table data for rendering
        table_data = latest_balances[['Hostname', 'Balance', 'Quil Per Day', '24-Hour Quil Per Day', 'Quil Per Minute', 'Quil Per Hour', '24-Hour Quil Per Hour', '$ Per Hour', '$ Per Day']].reset_index()
